...
import type { CategoricalPlotCategoryZoomParams, CategoricalPlotPluginParams } from '../types'
import { DEFAULT_CATEGORICAL_PLOT_CATEGORY_ZOOM_PARAMS } from '../defaults'
...
export const CategoryZoom = defineSVGLayer<CategoricalPlotExtendContext, CategoricalPlotPluginParams, CategoricalPlotCategoryZoomParams>({
  name: layerName,
  defaultParams: DEFAULT_CATEGORICAL_PLOT_CATEGORY_ZOOM_PARAMS,
  ...
  setup: ({ svgG, pluginParams$, layerParams$, context }) => {
    ...
    const categoryMaxIndex$ = context.computedData$.pipe(
      map(d => d[0] ? d[0].length - 1 : 0),  // category count - 1
      distinctUntilChanged()
    )
