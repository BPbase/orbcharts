...
interface BaseLineContext {
  ...
  baseLineParams$: Observable<BaseLineParams>
...
function highlightLine ({ selection, seriesLabel, styles }: {
...
export const createBaseLine: BaseLayerFn<BaseLineContext> = ({
  selection,
  pluginName,
  layerName,
  computedData$,
  ...
  const linePath$: Observable<d3.Line<ComputedLayoutDatumGrid>> = new Observable(subscriber => {
    const paramsSubscription = baseLineParams$
      .pipe(
        takeUntil(destroy$)
      )
      .subscribe(d => {
